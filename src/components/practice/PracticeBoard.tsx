        ...
        return (
            <div className="flex flex-col items-center justify-center min-h-[60vh] gap-10 w-full max-w-5xl mx-auto py-12">
                <div className="text-center space-y-4 mb-6">
                    <h2 className="text-5xl font-black tracking-tight text-neutral-900 dark:text-neutral-50">{t("practice_board_title")}</h2>
                    <p className="text-neutral-500 dark:text-neutral-400 text-lg max-w-2xl mx-auto">
                        {t("practice_board_subtitle")}
                    </p>
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-4 gap-6 w-full">
                    {[
                        { id: "basic", nameKey: "practice_mode_basic_name" as const, descKey: "practice_mode_basic_desc" as const },
                        { id: "intermediate", nameKey: "practice_mode_intermediate_name" as const, descKey: "practice_mode_intermediate_desc" as const },
                        { id: "advanced", nameKey: "practice_mode_advanced_name" as const, descKey: "practice_mode_advanced_desc" as const },
                        { id: "pro", nameKey: "practice_mode_pro_name" as const, descKey: "practice_mode_pro_desc" as const },
                    ].map(mode => (
                        <button
                            key={mode.id}
                        ...
                        >
                            <h3 className={cn("font-black text-2xl md:text-3xl mb-3", settings.gameMode === mode.id ? "text-blue-700 dark:text-blue-400" : "text-neutral-800 dark:text-neutral-100")}>{t(mode.nameKey)}</h3>
                            <p className="text-sm md:text-base text-neutral-500 dark:text-neutral-400 px-2 leading-relaxed">{t(mode.descKey)}</p>
                        </button>
                    ))}
                </div>
                ...
                <div className="flex flex-wrap items-center justify-center gap-4 w-full bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 p-4 rounded-xl shadow-sm">
                    <div className="flex items-center gap-2">
                        <span className="text-sm font-medium text-neutral-600 dark:text-neutral-300">{t("practice_decks_label")}</span>
                        <select
                    ...
                    {settings.gameMode === "intermediate" && (
                        <div className="flex items-center gap-2">
                            <span className="text-sm font-medium text-neutral-600 dark:text-neutral-300">{t("practice_cards_per_round")}</span>
                            <select
                                className="bg-neutral-100 dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 rounded-lg px-3 py-1.5 text-sm font-medium"
                    ...
                    {(settings.gameMode === "advanced" || settings.gameMode === "pro") && (
                        <div className="flex items-center gap-2">
                            <span className="text-sm font-medium text-neutral-600 dark:text-neutral-300">{t("practice_test_label")}</span>
                            <select
                                className="bg-neutral-100 dark:bg-neutral-800 border border-neutral-300 dark:border-neutral-700 rounded-lg px-3 py-1.5 text-sm font-medium"
                                value={settings.askFrequency}
                                onChange={(e) => updateSetting("askFrequency", e.target.value as "random" | "end")}
                            >
                                <option value="random">{t("practice_test_random")}</option>
                                <option value="end">{t("practice_test_end")}</option>
                            </select>
                        </div>
                    )}
                </div>
        ...
        <div className="flex flex-col items-center gap-6 w-full max-w-4xl mx-auto">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 w-full">
                <UICard>
                    <CardHeader className="p-3 pb-1">
                        <CardTitle className="text-xs text-neutral-500 dark:text-neutral-400 font-medium">{t("practice_streak")}</CardTitle>
                    </CardHeader>
                ...
                <UICard>
                    <CardHeader className="p-3 pb-1">
                        <CardTitle className="text-xs text-neutral-500 dark:text-neutral-400 font-medium">{t("practice_accuracy")}</CardTitle>
                    </CardHeader>
                ...
                {(settings.gameMode === "advanced" || settings.gameMode === "pro") && (
                    <div className="flex items-stretch gap-2">
                        <UICard className="border-blue-200 dark:border-blue-800 bg-blue-50 dark:bg-blue-950/30 flex-1">
                            <CardHeader className="p-3 pb-1">
                                <CardTitle className="text-xs text-blue-600 dark:text-blue-400 font-medium">{t("practice_running_count")}</CardTitle>
                            </CardHeader>
                        ...
                        >
                            {settings.mode === "training" ? t("practice_hide_count") : t("practice_show_count")}
                        </button>
                    </div>
                )}
                ...
                {settings.mode === "training" && settings.trueCountMode && (
                    <UICard className="border-indigo-200 dark:border-indigo-800 bg-indigo-50 dark:bg-indigo-950/30">
                        <CardHeader className="p-3 pb-1">
                            <CardTitle className="text-xs text-indigo-600 dark:text-indigo-400 font-medium">{t("practice_true_count")}</CardTitle>
                        </CardHeader>
            ...
            <div className="w-full space-y-1">
                <div className="flex justify-between text-xs text-neutral-500 dark:text-neutral-400">
                    <span>{actualCardsSeenCount} {t("practice_cards_seen")}</span>
                    <span>{totalShoeCards - actualCardsSeenCount} {t("practice_cards_remaining")}</span>
                </div>
                ...
                {gameState === "playing" && (
                    <div className="flex flex-col gap-4">
                        <Button
                            ...
                            autoFocus
                        >
                            {t("practice_next_card")} <ChevronRight className="ml-2 h-5 w-5" />
                        </Button>
                        {settings.hotkeys && (
                            <p className="text-center text-xs text-neutral-400">
                                {t("practice_hotkey_hint")}
                            </p>
                        )}
                    </div>
                )}
                    ...
                    <div className="space-y-4">
                        <p className="text-center font-medium text-neutral-600 dark:text-neutral-300">
                            <span className="block text-xs font-bold uppercase tracking-wider text-neutral-400 mb-1">
                                {settings.gameMode === "basic" ? t("practice_mode_label_basic") : t("practice_mode_label_intermediate")}
                            </span>
                            {t("practice_question_sum")}
                        </p>
                    ...
                    <div className="space-y-5 bg-white dark:bg-neutral-900 border border-neutral-200 dark:border-neutral-800 p-6 rounded-2xl shadow-sm">
                        <p className="text-center font-bold text-xl text-neutral-800 dark:text-neutral-100">
                            <span className="block text-xs font-bold uppercase tracking-wider text-neutral-400 mb-2 font-normal">
                                {settings.gameMode === "pro" ? t("practice_mode_label_pro") : t("practice_mode_label_advanced")}
                            </span>
                            {t("practice_question_running")} <span className="text-blue-600 dark:text-blue-400">{settings.gameMode === "pro" ? t("practice_true_count") : t("practice_running_count")}</span> {t("practice_question_running_end")}
                        </p>
                        {settings.gameMode === "pro" && (
                            <p className="text-center text-sm font-medium text-neutral-600 dark:text-neutral-300 bg-neutral-100 dark:bg-neutral-800 rounded-lg py-2">
                                {t("practice_decks_remaining")} <strong className="text-neutral-900 dark:text-white">{getDecksRemaining(shoe.length).toFixed(1)}</strong>
                            </p>
                        )}
                ...
                {gameState === "feedback" && feedback && (
                    ...
                    >
                        ...
                        <Button
                            ...
                            autoFocus
                        >
                            {t("practice_next_hand")} <ChevronRight className="ml-2 h-5 w-5" />
                        </Button>
                        {settings.hotkeys && (
                            <p className="text-xs opacity-60">{t("practice_enter_hint")}</p>
                        )}
                    </div>
                )}
